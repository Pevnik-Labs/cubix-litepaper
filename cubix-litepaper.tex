...
\title{Cubix: \\ A Next-Generation Blockchain with On-Chain Proofs}
\author{The Pevnik Labs Team}
\date{November 2025}
...
\begin{abstract}
Cubix is an attempt to design and prototype a next-generation blockchain architecture that has the potential to eventually replace the currently dominant Ethereum-style chains. We postulate a collection of novel features for this new blockchain:
...
We present the details of how these features fit together and how they provide remedies for known problems of existing blockchains. Although some of them are already present elsewhere, the overall vision is new. We conclude with an overview of Axi, Cubix's on-chain programming language.

Caution: this \textit{litepaper} sketches only the key ideas from ongoing research. It is different from the upcoming Cubix \textit{whitepaper}, which will contain detailed technical specifications.
\end{abstract}
...
We hope that the adoption of Cubix will challenge EVM dominance by offering significant usability gains and that, as a consequence, its ideas will spread to other chains, inspiring innovation and moving the industry forward.

The key distinguishing features of the Cubix blockchain are:
...
The following table gives a bird's-eye overview of key blockchain adoption obstacles (as listed above) vs features of Cubix blockchain that are supposed to mitigate them.

\bigskip
\begin{tabularx}{\textwidth}{|l|X|}
\hline
\textbf{Problem} & \textbf{Cubix's solution} \\
\hline
Bugs (exploited by hackers) & Cubix smart contracts are written in Axi, a purely functional programming language. Axi supports so-called ``concepts'' (concept is a language feature in Axi) which are semantically-enriched interfaces. In Axi one can use formal logic to prove that a software component is compliant with a given concept.
...
\hline
Limited privacy & For now the relevant features of Cubix are not decided yet. We are looking into existing solutions (such as Monero and Zcash) to estimate feasibility. \\
\hline
\end{tabularx}
\bigskip
...
Functional programming has one advantage over imperative: it is easier to reason about functional programs, in particular when doing reasoning with formal logic. This was the key reason why we decided to adopt the functional approach to on-chain programming in Cubix.
...
In Cubix, we adopt the opposite extreme: we enforce that all the on-chain code must be written in Axi. Moreover, we actually store on-chain the source code of Axi programs. Precisely speaking, code is stored as content-addressed elaborated AST:
...
Global state (``current state of the world'') in Cubix blockchain is composed of two parts:
...
This is exactly what we do in the Cubix blockchain:

\begin{itemize}
  \item code is in modules
  \item data is in cells
  \item every cell contains a single object --- this object is an instance of its ``class''; however, because Cubix on-chain programming is really functional programming in Axi, we rather say that this object is an element of its type (where the definition of the type sits in some module)
\end{itemize}

Smart contracts from Ethereum can be ported to the Cubix blockchain --- every smart contract would then correspond to a shared cell (here ``shared'' is a so-called \textbf{kind} of cell --- we describe kinds later in the next chapter). We just do not use the term ``smart-contract'' for a shared cell because the code defining the structure and behavior of the cell is not part of the cell (instead it is defined by functions in the corresponding module). We also do not want to use the term ``smart-contract'' meaning the module, because this could create even bigger confusion for people used to the Ethereum model (because in Cubix the closest thing to Ethereum smart contracts are really shared cells).
...
An \textbf{asset} pattern is for creating generally ``passive'' resources which can be traded/purchased: cryptocurrencies, NFTs, wrapped coins, soul-bound tokens. A built-in cryptocurrency of Cubix is also materialized using the asset pattern.
...
We want Cubix blockchain users to be as sovereign as possible. To achieve this we want every user to have an easy and maximally independent path for creating a blockchain account.
...
On Cubix, this is a little more complicated, because we have account abstraction. So for an account to be usable, the account object must be created first. For this to happen:
...
This situation is a little different in Cubix. On one hand we also have accounts ans services. But technically, both are represented as cells (account cells and shared cells). Hence we end up with the conclusion that in Cubix blockchain actors are just cells. In the Cubix homogenous model, cells serve all purposes.
...
\subsubsection{Delegation in Cubix blockchain}
...
We do not want to hardcode a fixed intent-based ecosystem into the Cubix blockchain. Rather, we believe the right way to approach the problem is to ensure that a blockchain provides a sufficiently versatile toolbox of mechanisms needed for building such ecosystems.
...
This means an i-market has to define a suitable format of an ``intent-type'' plugin. In Cubix we actually suggest how this can be accomplished. We call this ``the mandarin pattern''.
...
We want the authors of good on-chain code to be financially rewarded. Therefore Cubix blockchain supports two complementary mechanisms:
...
\begin{itemize}
  ...
  \item Bidding happens via invoking a \code{bid()} method of the auction mandarin. Bidding involves the actual transfer of the money (always in the form of Cubix native cryptocurrency). The money offered by the last bidder money gets locked in the auction mandarin. The bidding releases previously locked money (returns it back to the previous bidder).
...
In particular the Axi standard library will be deployed to the Cubix blockchain at Genesis in a form of a collection of packages. These packages will be deliberately donated to the public domain since day one.
...
Because royalties are paid as a fraction of the actually burned gas, package authors earn more if their code is less gas efficient. This is of course not a good correlation, because it looks like a violation of incentive compatibility. However, the situation is actually not that bad because the on-chain code in Cubix forms an open market. If there is a way to gas-optimize a popular package, someone may actually implement a gas-optimized version, deploy it and advertise its existence to the community, so that people will migrate their code to use the more efficient version.
...
In Cubix we put in place mechanisms to prevent both scenarios:
...
We have not decided yet which consensus protocol will be used in the Cubix blockchain. Most likely this will be a variant of proof-of-stake, which takes into account the native concurrency of the UTXO model.
