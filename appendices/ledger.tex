...
All Axi code deployed on the ledger is stored in a big codebase, divided into \textit{packages}. Package addresses are means of locating these packages in this codebase. Packages are further subdivided into smaller units, called \textit{modules}, which hold definitions of types, traits, constants and functions.
...
A new resource type is created by a transaction that deploys a package of Axi code that contains the definition of this resource type. The only exceptions are built-in resource types, including Gas, the native coin used to pay transaction fees, which already exist at the Genesis block. Because packages live on-chain forever and cannot be deleted, a resource type, once it comes to life, will also exist forever.

The life cycle of a particular resource, i.e. an element of some resource type, consists of three stages:

\begin{itemize}
  \item \textbf{Creation}. At some point the resource is created, in accordance with the interface of the package that contains the definition of the resource's type. After creation, the resource is placed on the ledger in a cell on its own or as a part of a bigger resource, owned by an account designated by its address, or in a shared cell.
  \item \textbf{Transaction(s)}. The transactor, as the owner, can transfer her ownership of the resource freely. Resources in shared cells can be altered through the interface set by the defining package of the resource type of a shared cell.
